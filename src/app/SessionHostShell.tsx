...
function selectedTab(): 'match' | 'history' | 'players' {
...
export function SessionHostShell({ userId, name, logoutButton, logoutFailed }: {
  ...
  return <div className="host-shell">
    ...
    <main>{tab === 'match' ? <SessionsPage key={userId} userId={userId} /> : tab === 'history' ? <HistoryPage key={userId} userId={userId} /> : <PlayersPage key={userId} userId={userId} />}</main>
    <nav className="host-nav" aria-label={t('players.navigation')}>
      ...
      <a className={tab === 'history' ? 'is-active' : undefined} href="#history" aria-current={tab === 'history' ? 'page' : undefined} onClick={() => setTab('history')}><img src="/assets/figma/history.svg" width="24" height="24" alt="" /><span>{t('players.history')}</span></a>
